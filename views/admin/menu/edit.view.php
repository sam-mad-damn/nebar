...
<div class="container justify-content-center">
    ...
    <div class="row justify-content-center p-3">
        <form action="/app/tables/admin/menu/updt.php" enctype="multipart/form-data" method="POST" class="menuForm">
            <div class="card col-8 text-center justify-content-center formCard" style="width: 23rem;">
            <input type="hidden" name="id" value="<?=$prod->id?>">
                <img src="/upload/<?=$prod->image?>" id="loadedImg" width="100" height="100" class="imgTable mt-3" alt="...">
                <div class="card-body">
                    <label for="image" class="mt-3">Изменить фото</label>
                    <input type="file" name="image" id="file" class="form-control">
                    <label for="name" class="mt-3">Изменить название</label>
                    <input type="text" class="form-control" name="name" value="<?=$prod->name?>"></input>
                    <?php if (!empty($_SESSION["error"]["name"])) : ?>
                        <p class="error"><?= $_SESSION["error"]["name"] ?></p>
                    <?php endif ?>

                    <label for="price" class="mt-3">Изменить цену за 100мл продукта</label>
                    <input type="number" class="form-control" name="price" value="<?=$prod->price?>"></input>
                    <?php if (!empty($_SESSION["error"]["price"])) : ?>
                        <p class="error"><?= $_SESSION["error"]["price"] ?></p>
                    <?php endif ?>

                    <label for="category" class="mt-3">Изменить категорию</label>
            ...
            <div class="card col-8 text-center justify-content-center formCard" style="width: 23rem;">
                <div class="card-body">
                    <textarea name="description" id="" class="form-control mt-3 " cols="30" rows="5"><?=$prod->description?></textarea>
                    <?php if (!empty($_SESSION["error"]["description"])) : ?>
                        <p class="error"><?= $_SESSION["error"]["description"] ?></p>
                    <?php endif ?>
                    <label class="mt-3">Изменить/добавить вес продукта</label>
